// app/dashboard/_components/FormList.jsx
    ...
    if (isLoading) {
        return (
            <div className='mt-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
                {/* Skeleton Loader */}
                {[1, 2, 3].map(item => (
                    <div key={item} className="h-[220px] bg-gray-200 dark:bg-gray-800 animate-pulse rounded-xl"></div>
                ))}
            </div>
        )
    }
    ...
    if (formList.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center mt-20 text-center border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-xl p-12">
              ...
              <p className="mt-2 text-gray-500 dark:text-gray-400">Click on the "+ Create Form" button to get started.</p>
            </div>
        );
    }
